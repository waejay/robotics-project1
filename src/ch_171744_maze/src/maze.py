...
class MazeSolver:
    ...
    def __init__(self):
        ...
        # Subscribe to get the data from laserscan and Odometry
        rospy.Subscriber('/laserscan', LaserScan, self.laserscan_callback)
        rospy.Subscriber('/odom', Odometry, self.odom_callback)

        # Publish to the turtlebot's velocity node
        ...
        self.distanceToWall = 0.4       # distance from the wall to the robot and min distance for obstacles detection
                                        # TODO: change this to 1.0 feet
        self.angle = 1.57               # around 90 degrees (from radians)
    ...
    def startSolver(self):
        ...
        while not rospy.is_shutdown():
            if(self.laser and self.odom): # laser and odom data arrived from callback
                ...
                actMinLaserValue = min(min(self.laser.ranges[170:190],
                                           self.laser.ranges[self.minLasersSide[0] : self.minLasersSide[1]]))

                # If TurtleBot detects a wall
                if(self.driveState == "WallDetection"):

                    # Halt linear and angular movement
                    self.vel.linear.x = 0.0
                    self.vel.angular.z = 0.0

                    self.wallDetection(actMinLaserValue)

                # If Turtlebot is currently driving to a wall
                    ...
                    if(self.mutex.locked() == False):
                        ...
                        if(actMinLaserValue <= self.distanceToWall):
                            ...
                            self.vel.linear.x = 0.3     # proceed to wall at a
                                                        # speed of 0.3
                            self.vel.angular.z = 0.0
                # If Turtlebot is currently following along a wall
    ...
    def wallDetection(self, actMinLaserValue):
        ...
        try:
            if(actMinLaserValue < self.distanceToWall):
                ...
                # if biggest wall in vicinity is NOT straight ahead
    ...
    def wallFollower(self, actMinLaserValue):
        ...
        if(actMinLaserValue < self.distanceToWall):
            self.rotate_angle(self.angle, self.turnSpeed)
        # obstacle along the robot
        elif(pidValue == 0):
            self.vel.linear.x = 0.3
            self.vel.angular.z = 0.0
        # obstacle gets farther away from robot (such as a corner)
        elif(pidValue != 0):
